...
gboolean update_video_direction(gpointer user_data) {
    CustomData *data = (CustomData *)user_data;
    gint dynamic_video_direction = 4; // Set dynamic video direction here

    g_object_set(G_OBJECT(data->videoflip), "video-direction", dynamic_video_direction, NULL);
    return G_SOURCE_CONTINUE;
}


gboolean increase_brightness(gpointer user_data) {
    CustomData *data = (CustomData *)user_data;
    gfloat brightness_value = 2.0; // Set dynamic video direction here

    g_object_set(G_OBJECT(data->videobalance), "saturation", brightness_value, NULL);
    return G_SOURCE_CONTINUE;
}
int main(int argc, char *argv[]) {
    ...
    data.videoflip = gst_element_factory_make("videoflip", "videoflip");
    ...
    if (!(data.streaming_pipe || data.src || data.depay || data.decoder || data.video_convert || data.videoflip || data.videobalance || data.encoder || data.sink)) {
...
//    g_object_set(G_OBJECT(data.videobalance), "saturation", 2.0, NULL);
    g_signal_connect(data.src, "pad-added", G_CALLBACK(on_rtsp_pad_added), &data);

    // Add and link elements to create the full pipeline
    gst_bin_add_many(GST_BIN(data.streaming_pipe), data.src, data.depay, data.decoder, data.video_convert, data.videoflip, data.videobalance, data.encoder, data.sink, NULL);
    if (!gst_element_link_many(data.depay, data.decoder, data.video_convert, data.videoflip, data.videobalance, data.encoder, data.sink, NULL)) {
...
//    guint timer_id = g_timeout_add(9000, update_video_direction, &data); // GSource to update video direction every second

    guint video_flip = g_timeout_add_seconds(10, update_video_direction, &data);

    guint video_brightness = g_timeout_add_seconds(13,increase_brightness, &data);
    ...
    g_source_remove(video_flip);
    g_source_remove(video_brightness);
